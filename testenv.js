...
const SRC = fs.readFileSync(path.join(__dirname, 'site-pure.user.js'), 'utf8');
...
function makeEnv(url) {
  ...
  const win = {
    location: (function () {
      var u = new URL(url || 'https://www.douyin.com/');
      return { href: u.href, pathname: u.pathname, search: u.search, hostname: u.hostname, host: u.host,
